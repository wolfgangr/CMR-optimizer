...
Maps from different sources (historic atlases in the focus) properly layered and manageable the qgis way  
...
Follow +- established procedure to process:
- load source image (*jp2) into GIMP, crop, export; optionally color enhance, scale
...
After running the georeferencer, we find a couple of extra files:
...
This file is **the focus of our interest** as input file for our optimisation
...
Manually reformated for human readability, a point file looks like:
```
#CRS:
PROJCRS["unknown",
  BASEGEOGCRS["unknown",
...
Note that the **WKT** aka [well-known text representation](https://en.wikipedia.org/wiki/Well-known_text_representation_of_coordinate_reference_systems)
of the CRS is reproduced from and thus **semantically** roughly **equivalent** to the "good old" **`proj` notation**:  
`+proj=bonne +lat_1=50 +lon_0=20 +ellps=WGS84 +units=m +no_defs +type=crs`

The **CSV block** with the parameters of the **matchpoints** we entered is without spaces. It's content matches the table as visible in the georeferencer's GUI.  
`mapX`, `mapY` are the coordinates in the **target CRS** provided - even if we entered them manually as plain integer values in `WGS 84` CRS.  
`sourceX`, `sourceY` are the **pixel positions** in the source raster where we placed our match points in the GUI.  

N.B.: If we change our georef target CRS, `mapX`, `mapY` will change, even if the underlying lat/lon does not change.  
...
The site provides a quite concise, nontheless technically quite complete and correct overview of classifications, including:
...
[List of all projection images](https://proj.org/en/stable/operations/projections/all_images.html) and served as entry point into proj for the work presented here.

### ESRI "Understanding Map Projections"
pdf by Melita Kennedy: [Understanding Map Projections](http://kartoweb.itc.nl/geometrics/Map%20projections/Understanding%20Map%20Projections.pdf)
...
pdf by [U.S. Geological Survey professional paper 1453](https://pubs.usgs.gov/pp/1453/report.pdf) - First printing 1989, Second printing 1994

similiar, but at intermediary technical level, only black-white images, but including formulas  
Not really necessary since we rely on `proj`, but nevertheless helpful for indepth understanding.


## 4) Playing with Georef transformation settings
... and try to understand them ...  
We find **circular parallels** on our source map. Thus we might head for `eqdc` aka *equidistant conic*.  
A closer look reveals slightly **bent meridians**, too, so we'd go for `LAEA` *(equal area azimuthal)* or it's bretheren instead.  
Searching QGIS standard CRS list, we find **EPSG:3575** *"europe North pole LAEA"* as close enough and use this as **georef target CRS** to start whith.

### 4 a) Helmert
Helmert transformation just allows **shift, scale and rotation** of the source, to end at target CRS.  
Georeferencer calculates more than **80 Pixel error** (i.e. the average deviation from calculated pixel by reverting the transformation to the mark clicked) - clearly visible as red lines in the georef UI:

![](Shepherd_0042-Georef-Helmert.jpg)  

In the `geotiff` generated by the georeferencer, we find our image rotated  
*(n.b.: I converted the geotif to jpg to save storage and bandwith.  )*  
...
Pixel error falls to 52, leaving still quite some long red needles in the map:  

![](Shepherd_0042-Georef-Poly1.jpg)  

There is little visual difference to helmert in the geotif, but when we toggle between them in an image viewer, we may watch the change of vertical:horizontal ratio:  
...
When we switch to **Polynomial 2**, pixel error drastically falls from 52 to 3.5.  
...
It might 'somehow looks like' a wrapped conus, but not really.  
...
However, compared to poly 2, we merely see **pixel error fall from 3.5 to 3.2**.  
The difference is close to invisible in the UI.
...
In QGIS main window (Georeferncer window still active), we find the graticule's crossings neatly placed at the red dots.  
...
Pixel error as schjown by the georeferencer falls to some negative exponential I'd consider as zero.  
...
In _italics_, we include the hypothetical map making process to point out the the artefacts we try to revert.

- *assume some **precise** source data maps, derived from 19th ctry **survey** data, covered in the late map makers fundus*
- *the map maker applies some **projection** to those maps from a **set** we might hope to know*
- *manual processing, pre-print and **print** operations some decades ago may produce extra errors*
...
What we change **in our approach** ist that we employ **least square** algorithm not to the arbitrary polynomial transformation, but to support the **informed selection and parametrisation** of the assumed **raster source CRS**. 

We expect (and found) that the **residual** polynomial **transformation** necessary is **substantially simplified**.
...
we employ least square based optimisation to the choice of projection and their parameters and apply only minor corrections to the results.
...
Instead, we expect the **center of projection**, where the projection plane touches the sphere, somewhere close to, presumably **inside our maps area**.  
...
We extend our script to provide for rotation. Sadly, for reasons not yet scrutinized, it refuses to converge then.  
...
Andy why would a mapmaker decide for this somewhat odd figure as 20.44?
...
The known arguments for [LAEA](https://en.wikipedia.org/wiki/Lambert_azimuthal_equal-area_projection) (visual weight of remote areas, complete coverage of the globe) do not hold for the assumed purpose of the map at hand. Might we question a decision of the late map maker for this choice?
...
With the understanding gained 'till now, we reread our documentations.  
In the USGS paper cited above, we find [Bonne's projection](https://proj.org/en/stable/operations/projections/bonne.html#bonne) mentioned as _"frequently used until recently for atlas maps of continents"_. Looks like it had escaped out attention before due to it's weird shape of world maps boundaries. Hower, in areas cropped to the size of continents, Bonne looks quite familiar and close to our source image.  

And similiar to azimutal views, Bonne takes a **map center**, expressed e.g. in lat/lon, **as parameters**.  
...
When we look at the transformed geotiff, even in the case of 3rd degree polynomial transformation, the difference to a linear helmert style aka rectangular transformation is barely visible:
...
As expected, the georeferencer refuses 2nd or 3rd order polynomial transformation due to a lack of point number.
